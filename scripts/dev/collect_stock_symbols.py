...
데이터 소스: pykrx 라이브러리
저장 위치: config/symbols/kr_all_symbols.txt (프로젝트 루트 기준)
"""
...
def save_cache(symbols: list, output_path: Path = None):
    ...
    if output_path is None:
        # config/symbols/kr_all_symbols.txt (repo root 기준)
        _repo_root = Path(__file__).resolve().parents[2]
        output_path = _repo_root / "config" / "symbols" / "kr_all_symbols.txt"
